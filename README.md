...
> 💡 **Идея простая:** сначала подготовь всё на компе, потом ставь на сервер.
> Claude Code на VPS поможет донастроить, но ему нужны готовые ключи и форк.

---

## 🎯 Шаг 1: Зарегистрируйся на GitHub и сделай Fork

**Без этого дальше никуда** - форк это твоя личная копия проекта.

1. Зайди на [github.com](https://github.com) → "Sign up" → создай аккаунт
2. Открой [github.com/smixs/agent-second-brain](https://github.com/smixs/agent-second-brain)
3. Нажми кнопку **Fork** (правый верхний угол)
4. Выбери свой аккаунт → "Create fork"
...
✅ Теперь у тебя есть: `https://github.com/ТВОЙ_ЛОГИН/agent-second-brain`

---

## 🔑 Шаг 2: Получи ВСЕ ключи заранее

**Собери это ДО покупки VPS.** Потом просто вставишь готовые значения.

### 2.1 Telegram Bot Token
1. Telegram → найди `@BotFather` → отправь `/newbot`
2. Придумай имя и username (заканчивается на `bot`)
3. Скопируй токен: `7123456789:AAHxxxxxxxxxx`
...
### 2.2 Твой Telegram ID
1. Telegram → найди `@userinfobot` → отправь любое сообщение
2. Скопируй свой ID: `123456789`

### 2.3 Deepgram API Key
1. Зайди на [console.deepgram.com](https://console.deepgram.com/)
2. Зарегистрируйся → Settings → API Keys → Create
...
1. Зайди на [todoist.com](https://todoist.com)
...
- [ ] Fork URL: `https://github.com/ТВОЙ_ЛОГИН/agent-second-brain`
- [ ] Telegram Bot Token: `7123456789:AAHxxxxxxxxxx`
- [ ] Твой Telegram ID: `123456789`
- [ ] Deepgram API Key: `xxxxxxxxxxxxxxxx`
- [ ] Todoist API Token: `xxxxxxxxxxxxxxxx`

---

## 🖥️ Шаг 3: Купи VPS и подключись

Теперь, когда всё готово - ставим сервер.

1. Купи VPS на [Contabo](https://contabo.com/en/vps/) или [OVH](https://ovhcloud.com/en/vps/)
2. Выбери Ubuntu 22.04
3. Получи на почту IP-адрес и пароль
...
4. Открой терминал и подключись:
...
## ⚙️ Шаг 4: Установи базовые инструменты

Копируй команды по одной:
...
## 🔐 Шаг 6: Авторизуйся в Claude

```bash
claude auth login
```

Следуй инструкциям: откроется ссылка → авторизуйся в браузере → скопируй код → вставь в терминал.
...
## 🚀 Шаг 7: Запусти Claude Code - он доделает остальное
...
У меня готово:
- Fork: https://github.com/МОЙ_ЛОГИН/agent-second-brain
- Telegram Bot Token: [вставь]
- Telegram ID: [вставь]
- Deepgram Key: [вставь]
- Todoist Token: [вставь]

Установи Python, uv, клонируй мой форк и настрой бота.
```

**Всё.** Claude Code прочитает инструкцию, возьмёт твои ключи и сделает всё сам - установит Python, клонирует форк, создаст .env, настроит автозапуск.

Если что-то пойдёт не так - просто опиши проблему. Он разберётся.
